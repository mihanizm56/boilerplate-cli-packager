...
const commandsMap = {
  ...
  [buildCommand]:
    'set CI=true && npx cross-env NODE_ENV=production PUBLIC_URL=/ npx react-app-rewired build && node ./config/deploy/make-runtime-config.js',
  ...
  [deployToTest]: `bash ./config/deploy/template_ci.sh ${repoName} ${deployToken} ${namespace} test`,
  [deployToStage]: `bash ./config/deploy/template_ci.sh ${repoName} ${deployToken} ${namespace} stage `,
  [deployToTestsStands]: `bash ./config/deploy/template_ci.sh ${repoName} ${deployToken} ${namespace} test stage`,
  [deployToAllStands]: `bash ./config/deploy/template_ci.sh ${repoName} ${deployToken} ${namespace} test stage dataline datapro`,
};
