const { projectName } = require('../_constants');
...
// const buildDocker = 'build-docker';
...
const commandsMap = {
  ...
  [deployToTest]: `node ./cli/_utils/ci-utils/executor.js --command=${fullCheckSystem} && ./config/deploy/template_ci.sh test ${projectName}`,
  [deployToStage]: `node ./cli/_utils/ci-utils/executor.js --command=${fullCheckSystem} && ./config/deploy/template_ci.sh stage ${projectName}`,
  [deployToDataline]: `node ./cli/_utils/ci-utils/executor.js --command=${fullCheckSystem} && ./config/deploy/template_ci.sh dataline ${projectName}`,
  [deployToDatapro]: `node ./cli/_utils/ci-utils/executor.js --command=${fullCheckSystem} && ./config/deploy/template_ci.sh datapro ${projectName}`,
  // [buildDocker]: `sudo docker build -t test -f ./config/deploy/Dockerfile .`,
