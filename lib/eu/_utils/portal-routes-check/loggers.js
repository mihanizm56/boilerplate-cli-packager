...
module.exports.getOutdatedPackageLog = ({
  ...
  console.log(
    `(${'!'.red})${
      ` I've have found that you are using not the latest version of ${ROUTES_PACKAGE_NAME} package "${packageName}"`
        .yellow.underline
    }`,
  );
  console.log('');
  ...
  console.log('');
  console.log(
    `npm install --no-audit --legacy-peer-deps ${packageName}@${latestVersion} -E`
      .yellow,
  );
};
...
module.exports.getActualSuccessPackageLog = ({ packageName, version }) => {
  console.log(
    `You are using the latest version of ${ROUTES_PACKAGE_NAME} package "${packageName}"`
      .green.underline,
  );
  console.log('');
