...
const commandsMap = {
  ...
  [integrationTestsCommand]:
    'node ./config/env/make-runtime-config.js && npx cross-env SERVER_PORT=5005 IP_LIMIT=1000 testcafe chrome:headless ./integration-tests -a "node ./config/ink/_utils/ci-utils/executor.js --command=start"',
  [testCoverateCommand]: 'set CI=true && npx jest --coverage',
  [fullCheckCommand]: `node ./config/ink/_utils/ci-utils/executor.js --command=${testCommand} && node ./config/ink/_utils/ci-utils/executor.js --command=${fullLintCheckCommand} && git add . && node ./config/ink/_utils/ci-utils/executor.js --command=${checkNPMCommand} && node ./config/ink/_utils/ci-utils/executor.js --command=${buildCommand} && node ./config/ink/_utils/ci-utils/executor.js --command=${integrationTestsCommand}`,
